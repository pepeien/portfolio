...
// Components
import { default as WorldMap, Languages as WorldMapLanguages } from '@components/WorldMap';
...
export default function Component({ dictionary, locales }: Props) {
    ...
    const updateCurrentLanguage = (value: string) => {
        setCurrentLanguage(
            Object.keys(WorldMapLanguages).find(
                (_) => value.includes(_.toLowerCase()) || value.includes(_.toUpperCase()),
            ) ?? 'EN',
        );
    };
    ...
    return (
        <React.Fragment>
            <button className='language-action' onClick={() => setIsListVisible(!isListVisible)}>
                <svg
                    version='1.0'
                    id='Layer_1'
                    ...
                    <ul className='--flex-row'>
                        {Object.entries(locales).map(([id, name]) => (
                            <li
                                ...
                                onMouseEnter={() => updateCurrentLanguage(id)}
                                onMouseLeave={() =>
                                    updateCurrentLanguage(dictionary['LANGUAGE_LOCALE'])
                                }
                                onFocus={() => updateCurrentLanguage(id)}
                                onBlur={() => updateCurrentLanguage(dictionary['LANGUAGE_LOCALE'])}
                            >
                                <Link
                                    className='--flex-row'
